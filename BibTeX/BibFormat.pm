...
# MODIFIED   : 
# VERSION    : $Id: BibFormat.pm,v 1.5 1999/03/11 04:54:06 greg Exp $
...
The F<Text::BibTeX::BibFormat> class is a base class of
F<Text::BibTeX::BibEntry> for formatting bibliography entries.  It thus
...
class, F<Text::BibTeX::BibSort>.  

F<BibFormat> (the F<Text::BibTeX> prefix will be dropped for brevity)
...
control what language is generated by F<BibFormat>; if none of those
options are set, then it will generate plain, unmarked text.

The only method in F<BibFormat>'s documented interface (so far) is
...
sub format_names
{
   ...
   $format->set_text (&BTN_FIRST, post_token => '')
...
sub format_article
{
   ...
   my $journal_block = [[$self->get ('journal'),
